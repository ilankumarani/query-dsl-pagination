...
public class SUsers extends com.querydsl.sql.RelationalPathBase<BUsers> {
    ...
    public void addMetadata() {
        addMetadata(active, ColumnMetadata.named("ACTIVE").withIndex(1).ofType(Types.BOOLEAN).withSize(1).notNull());
        addMetadata(email, ColumnMetadata.named("EMAIL").withIndex(4).ofType(Types.VARCHAR).withSize(255));
        addMetadata(fullName, ColumnMetadata.named("FULL_NAME").withIndex(5).ofType(Types.VARCHAR).withSize(255));
        ...
        addMetadata(registeredAt, ColumnMetadata.named("REGISTERED_AT").withIndex(3).ofType(Types.TIMESTAMP).withSize(26).withDigits(6));
